...
const CHECKED_SOURCE_FILE_EXTENSIONS = [
  "ts",
  "tsrx",
  "mts",
  "cts",
...
const FULL_CHECK_FILE_PATTERNS = FULL_CHECK_FILE_GLOBS.flatMap(
  compileFileFilterGlobs,
);
...
function compileFileFilterGlobs(fileGlob: string) {
  return [
    compileFileFilterGlob(fileGlob),
    compileFileFilterGlob(`**/${fileGlob}`),
  ];
}
...
function findMainPushSha(remoteName: string, updates: PushUpdate[]) {
  ...
  return updates.find(
    ({ localRef, remoteRef }) =>
      remoteRef === "refs/heads/main" && localRef !== "(delete)",
  )?.localSha;
}
...
function getChangedFiles(
  ...
  const result = spawnSync(
    "git",
    ["diff", "--name-only", `${branchPoint}..${pushedSha}`],
    {
      cwd: repoRoot,
      encoding: "utf8",
    },
...
function shouldRunRepoCheck(changedFiles: string[]) {
  return changedFiles.some((file) =>
    FULL_CHECK_FILE_PATTERNS.some((pattern) => pattern.test(file)),
  );
}
...
function runRepoCheck(repoRoot: string) {
  return (
    runBun(repoRoot, ["install", "--frozen-lockfile"]) &&
    runBun(repoRoot, ["check"])
  );
}

const [remoteName = ""] = process.argv.slice(2);
const pushedSha = findMainPushSha(
  remoteName,
  parsePushUpdates(await readStdinText()),
);
...
if (!hasOriginMain(repoRoot)) {
  process.exit(runRepoCheck(repoRoot) ? 0 : 1);
}
