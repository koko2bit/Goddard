...
- **Data Shape:** Normalized proposal records keyed by proposal id; ordered visible ids; filters for repository, status, owner, and query; selected proposal id; loading and error state.
...
- **Side Effects:** Fetches proposal records through shared app or SDK adapters; routes proposal detail openings through `WorkbenchTabsState`; keeps roadmap interactions aligned with the same cross-repository data model used elsewhere in the app.
