    ...
    switch (spec.kind) {
      case "chip": {
        ...
        items.push({
          ...
          text: spec.label,
          width:
            Math.ceil(measureSingleLineWidth(prepared)) + CHIP_CHROME_WIDTH,
        });
        pendingGap = 0;
        break;
      }
      ...
      case "text": {
        ...
        items.push({
          ...
          leadingGap:
            carryGap > 0 || hasLeadingWhitespace ? INLINE_BOUNDARY_GAP : 0,
          prepared,
        });
        break;
      }
...
function render(): void {
  ...
  const maxBodyWidth = Math.max(
    ...
    Math.min(
      BODY_MAX_WIDTH,
      viewportWidth - PAGE_MARGIN * 2 - NOTE_SHELL_CHROME_X,
    ),
  );
  const bodyWidth = Math.max(
    BODY_MIN_WIDTH,
    Math.min(maxBodyWidth, requestedWidth),
  );
  ...
  const noteBodyHeight =
    lineCount === 0
      ...
      : (lineCount - 1) * LINE_HEIGHT + LAST_LINE_BLOCK_HEIGHT;
