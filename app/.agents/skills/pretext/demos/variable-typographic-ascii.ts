...
function render(now: number): void {
  ...
  const attractor2X =
    Math.cos(now * 0.0013 + Math.PI) * CANVAS_W * 0.2 + CANVAS_W / 2;
  const attractor2Y =
    Math.sin(now * 0.0009 + Math.PI) * CANVAS_H * 0.25 + CANVAS_H / 2;
  ...
  sCtx.drawImage(
    ...
    attractor1X - LARGE_ATTRACTOR_R,
    attractor1Y - LARGE_ATTRACTOR_R,
  );
  sCtx.drawImage(
    getSpriteCanvas(ATTRACTOR_R),
    attractor2X - ATTRACTOR_R,
    attractor2Y - ATTRACTOR_R,
  );
    ...
    for (let col = 0; col < COLS; col++) {
      ...
      for (let sampleY = 0; sampleY < FIELD_OVERSAMPLE; sampleY++) {
        const sampleRowOffset =
          fieldRowStart + sampleY * FIELD_COLS + fieldColStart;
