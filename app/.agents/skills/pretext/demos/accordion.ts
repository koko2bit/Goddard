...
function getAccordionItemNodes(list: HTMLElement): AccordionItemDom[] {
  const roots = Array.from(
    list.querySelectorAll<HTMLElement>(".accordion-item"),
  );
...
function render(_now: number): boolean {
  ...
  const paddingY =
    parsePx(innerStyles.paddingTop) + parsePx(innerStyles.paddingBottom);

  let openItemId = st.openItemId;
  if (st.events.clickedItemId !== null) {
    openItemId =
      openItemId === st.events.clickedItemId ? null : st.events.clickedItemId;
  }
  ...
  for (let index = 0; index < items.length; index++) {
    const metrics = layout(
      preparedCache.items[index]!,
      contentWidth,
      lineHeight,
    );
    panelHeights.push(Math.ceil(metrics.height + paddingY));
    panelMeta.push(
      `Measurement: ${metrics.lineCount} lines · ${Math.round(metrics.height)}px`,
    );
  }
