...
function getInitialChatWidth(): number {
  ...
  const parsed =
    datasetValue === undefined ? Number.NaN : Number.parseInt(datasetValue, 10);
...
function updateBubbles(chatWidth: number): void {
  ...
  domCache.root.style.setProperty(
    "--bubble-max-width",
    `${renderState.bubbleMaxWidth}px`,
  );
  ...
  domCache.cssWaste.textContent = formatPixelCount(
    renderState.totalWastedPixels,
  );
