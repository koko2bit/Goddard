...
const pullquoteSpecs: PullquoteSpec[] = [
  {
    ...
    placement: { colIdx: 0, yFrac: 0.48, wFrac: 0.52, side: "right" },
  },
  {
    prepared: preparedPullquotes[1]!,
    placement: { colIdx: 1, yFrac: 0.32, wFrac: 0.5, side: "left" },
  },
];
...
stage.addEventListener("pointerdown", (event) => {
  ...
  const activeOrbCount =
    window.innerWidth < NARROW_BREAKPOINT ? NARROW_ACTIVE_ORBS : st.orbs.length;
  const radiusScale =
...
function render(now: number): boolean {
  ...
  const activeOrbCount = isNarrow
    ? Math.min(NARROW_ACTIVE_ORBS, st.orbs.length)
    : st.orbs.length;
  const orbs = st.orbs;
  ...
  const hoveredOrbIndex = hitTestOrbs(
    orbs,
    pointer.x,
    pointer.y,
    activeOrbCount,
  ...
  const cursorStyle =
    drag !== null ? "grabbing" : hoveredOrbIndex !== -1 ? "grab" : "";
