...
const { lines } = layoutWithLines(prepared, 320, 26); // 320px max w < lines.length; i++) ctx.fillText(lines[i].text, 0, i * 26)
```
...
layoutWithLines(prepared: PreparedTextWithSegments, maxWidth: number, lineHeight: number): { height: numb, lineCount: number, lines: LayoutLine[] } // high-level api for manual layout needs. Accepts a fixed max width for all lines. Similar to `layout()`'s return, but additionally returns the lines info
