...
function buildDebugMessages(): SessionTranscriptItem[] {
  const messages: SessionTranscriptItem[] = [
    {
      ...
      content: [{ type: "text", text: systemMessageBodies[0] }],
    },
    {
      kind: "toolCall",
      id: "tool:opening",
  ...
  for (let index = 0; index < 96; index += 1) {
    const role =
      index % 11 === 0 ? "system" : index % 2 === 0 ? "assistant" : "user";
    const authorName =
      role === "assistant" ? "Goddard" : role === "user" ? "Alec" : "Runtime";
    const body =
      role === "assistant"
    ...
    if (role === "assistant" && index % 24 === 10) {
      messages.push({
        ...
        title:
          index % 48 === 10
            ? "Search layout references"
            : "Execute transcript probe",
