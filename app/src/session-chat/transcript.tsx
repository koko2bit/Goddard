...
function estimateToolGroupRowHeight(
  ...
  const approximateCharactersPerLine = Math.max(
    18,
    Math.floor(summaryWidth / 8),
  );
  const joinedTitles = toolCalls.map((toolCall) => toolCall.title).join(", ");
  const lineCount = Math.max(
    1,
    Math.ceil(joinedTitles.length / approximateCharactersPerLine),
  );
...
function TranscriptRow(props: { row: MessageListRow<RenderTranscriptItem> }) {
  if (props.row.item.kind === "toolGroup") {
    return (
      <ToolTranscriptRow
        groupId={props.row.item.id}
        toolCalls={props.row.item.toolCalls}
      />
    );
  }
  ...
  return (
    <TextTranscriptRow
      row={props.row as MessageListRow<SessionTranscriptTextMessage>}
    />
  );
}
...
export function Transcript(props: TranscriptProps) {
  ...
  const renderItems = useMemo(
    () => coalesceTranscriptItems(props.messages),
    [props.messages],
  );
